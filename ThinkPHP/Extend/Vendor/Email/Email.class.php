...
/*
 * @author: Li Jingyi
 *	@Date: 2014-4-27 1:59:44
 *	@What's new:
 *	--把返回值由bool型改为了$msg数组，方便将错误信息以json的格式输出
 *	--利用缓冲区将phpmailer类中输出的错误信息接住并赋至：
 *	--$msg['message']中。
 *	@300代表错误,200代表成功
 */
...
class Email
{
	...
	static public function send($mail_to,$mail_subject,$mail_body,$mail_attach=NULL)
	{
		...
		if(!$mail->Send()) 
		{
			self::$msg['message'] = "发送失败:<br>".ob_get_contents()."<br>Mailer Error: " . $mail->ErrorInfo;
			ob_clean();
		  	return self::$msg;
		} 
		else 
		{
			self::$msg['statusCode'] = "200";
			self::$msg['message'] = "邮件发送成功！";
		    return self::$msg;
		}
	}
}
